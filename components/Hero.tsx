...
const Hero = () => {
    return (
        <div className="container relative flex max-lg:flex-col justify-between h-[calc(100vh-8rem)] max-lg:h-auto items-center">
            <div className="w-full flex flex-col max-lg:items-center max-lg:pt-16!">
                ...
                <div className="flex flex-col gap-4 mt-[1.6rem]! animate-slide-in-left-150">
                    <h1 className="max-lg:text-center">UI doesn&apos;t sell. <br /> <Typewriter text="Code does." delay={150} /></h1>
                    <p className="max-w-[48ch] max-lg:text-center">Clean UI is nice — fast, predictable code is what drives conversion. <br /> I build the code behind templates, plugins, digital products, and anything that drives conversions.</p>
                </div>
                <div className="mt-16! flex items-center gap-[2.4rem] max-w-[48ch] max-lg:w-full animate-slide-in-up-300">
                    <Button className="w-full text-center" variant="gradient" href="/#book-a-call">Book a call</Button>
                    <Button className="w-full text-center" variant="secondary" href="/#work">My Work</Button>
                </div>
                <div className="w-7xl h-276 rounded-full absolute top-1/2 -left-250 -translate-y-1/2 -z-1 
        bg-(--primary-700)/70 blur-[40rem]"></div>
                <div className="flex items-center gap-[1.6rem] mt-16!">
                    <div className="flex items-center animate-slide-in-left-450">
                        <div className="rounded-full overflow-hidden w-fit border border-(--white)">
                            <Image className="pointer-events-none" src={selfie_1} alt="Selfie 1" width={36} height={36} objectFit="cover" />
                        </div>
                        <div className="rounded-full overflow-hidden w-fit border border-(--white) -ml-3!">
                            <Image className="pointer-events-none" src={selfie_2} alt="Selfie 2" width={36} height={36} objectFit="cover" />
                        </div>
                        <div className="rounded-full overflow-hidden w-fit border border-(--white) -ml-3!">
                            <Image className="pointer-events-none" src={selfie_3} alt="Selfie 3" width={36} height={36} objectFit="cover" />
                        </div>
                        <div className="rounded-full overflow-hidden w-fit border border-(--white) -ml-3!">
                            <Image className="pointer-events-none" src={selfie_4} alt="Selfie 4" width={36} height={36} objectFit="cover" />
                        </div>
                    </div>
                    <div className="relative animate-slide-in-left-450">
                        <span>Trusted by people in eCommerce niche</span>
                        <Image src={vector} alt="Line vector" className="absolute -bottom-3 left-0" />
                    </div>
                </div>
            </div>
            <div className="relative self-end w-full animate-slide-in-right-200 ">
                <Image src={my_selfie} alt="Bogdan Stevanovic - Fullstack Web Developer" quality={100} className="max-lg:w-[80vh] max-lg:mx-auto!" />
                <div className="absolute inset-0 bg-linear-to-t from-(--primary-900) to-60% to-transparent z-1"></div>
                <div className="w-6xl max-xl:w-3xl max-md:w-xl aspect-square bg-(--primary-900) border-[.4rem] border-(--primary-700) rounded-full 
                absolute top-1/2 left-1/2 -translate-1/2
                -z-2"></div>
                <div className="w-4xl max-xl:w-2xl max-md:w-lg aspect-square bg-(--primary-900) border-[.4rem] border-(--primary-700) rounded-full 
                absolute top-1/2 left-1/2 -translate-1/2
                -z-2"></div>
                <div className="w-2xl max-xl:w-1xl max-md:w-md aspect-square bg-(--primary-900) border-[.4rem] border-(--primary-700) rounded-full 
                absolute top-1/2 left-1/2 -translate-1/2
                -z-2"></div>
                <div className="w-sm max-xl:w-sm aspect-square bg-(--primary-400) rounded-full blur-3xl
